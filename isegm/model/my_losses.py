...
class AttWeightLoss(nn.Module):
    ...
    def forward(self, pred, gt, pos_click_len):
        ...
        for i in range(len(pred)):
            ...
            tmp_loss[mask, :, :] = 0.01 * tmp_loss[mask, :, :]     # 点击次数等于1次的不计算损失
            loss = loss + torch.mean(tmp_loss)

        return loss/len(gts)

class DiscriminateiveAffinityLoss(nn.Module):
    def __init__(self, class_num=None, loss_index=[False,False,False,False], scale=1, crop_size=512):
        ...
        strides = [4, 8, 16, 32]
        self.sizes = [crop_size // (stride * scale) for stride in strides]
    ...
    def forward(self, seg, attns, attnWs, pos_click_len):
        ...
        for idx, i in enumerate(self.stages):
            ...
            loss_tmp[mask, :, :] = 0.01 * loss_tmp[mask, :, :]
